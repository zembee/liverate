(Datafeeds = {}),
  (Datafeeds.UDFCompatibleDatafeed = function() {
    ...
    (this._configuration = void 0),
      ...
      (this._callbacks = {}),
      (this._binary_websockets = new BinaryWebsockets()),
      (this._symbolRequestResponseHandler = new SymbolReqRespHandler(
        this._binary_websockets
      )),
      (this._historicalOHLCReqResHandler = new HistoricalOHLCReqRespHandler(
        this._binary_websockets
      )),
      (this._ohlcStreamingReqResHandler = new OHLCStreamingReqResHandler(
        this._binary_websockets,
        this._symbolRequestResponseHandler
      )),
      (this._supported_resolutions = []),
      this._supported_resolutions.push("1"),
      this._supported_resolutions.push("2"),
      this._supported_resolutions.push("3"),
      this._supported_resolutions.push("5"),
      this._supported_resolutions.push("10"),
      this._supported_resolutions.push("15"),
      this._supported_resolutions.push("30"),
      this._supported_resolutions.push("60"),
      this._supported_resolutions.push("120"),
      this._supported_resolutions.push("240"),
      this._supported_resolutions.push("480"),
      this._supported_resolutions.push("D"),
      (this.globalNotifier = GlobalNotifier.getInstance()),
      this._initialize();
  }),
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype.on = function(a, b) {
    "use strict";
    return (
      this._callbacks.hasOwnProperty(a) || (this._callbacks[a] = []),
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
    ...
    this._binary_websockets.init().then(function(b) {
      b &&
        a._symbolRequestResponseHandler.init().then(function(b, c) {
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype.onReady = function(a) {
    ...
    setTimeout(function() {
      b._configuration
        ? a(b._configuration)
        : b.on("configuration_ready", function() {
            a(b._configuration);
          });
    }, 0);
  }),
  (Datafeeds.UDFCompatibleDatafeed.prototype._setupWithConfiguration = function(
    ...
    if (
      ((a.symbols_types = c), !a.supports_search && !a.supports_group_request)
    )
    ...
    a.supports_search ||
      ...
      a.supports_group_request
        ? (this._symbolsStorage = new Datafeeds.SymbolsStorage(this))
        : this.onInitialized(),
      this._fireEvent("configuration_ready"),
      this._logMessage("Initialized with " + JSON.stringify(a));
  }),
    ...
    if (this._configuration.supports_search) {
      var e = this,
        f = function(a) {
          var b = [];
          return (
            $.each(e._symbolRequestResponseHandler._markets, function(d, f) {
              f.name === c &&
                $.each(f.submarkets, function(d, g) {
                  $.each(g.symbols, function(d, g) {
                    f.name.indexOf(c) !== -1 &&
                      ((g.symbol.indexOf(a) === -1 &&
                        g.symbol_display.toUpperCase().indexOf(a) === -1) ||
                        b.push({
                          symbol: g.symbol,
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype.searchSymbolsByNameOnly = function(
    a
  ) {
    "use strict";
    ...
    if (this._configuration.supports_search) {
      var b = this,
        c = [];
      return (
        $.each(b._symbolRequestResponseHandler._markets, function(d, e) {
          $.each(e.submarkets, function(d, f) {
            $.each(f.symbols, function(d, f) {
              (f.symbol.indexOf(a) === -1 &&
                f.symbol_display.toUpperCase().indexOf(a) === -1) ||
                c.push({
                  symbol: f.symbol,
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(a, b, c) {
    ...
    setTimeout(function() {
      ...
      if (!d._initializationFinished)
        return void d.on("initialized", function() {
          d.resolveSymbol(a, b, c);
        });
      if (d._configuration.supports_group_request)
        d._initializationFinished
          ? d._symbolsStorage.resolveSymbol(a, e, c)
          : d.on("initialized", function() {
              d._symbolsStorage.resolveSymbol(a, e, c);
            });
      else {
        var f = !1;
        $.each(d._symbolRequestResponseHandler._markets, function(b, c) {
          return (
            $.each(c.submarkets, function(b, g) {
              return (
                $.each(g.symbols, function(b, g) {
  ...
  (Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(
    ...
    "use strict";
    this.globalNotifier.loadingNotification(),
      this._historicalOHLCReqResHandler.getBars(a, e, f);
  }),
    ...
    $.each(a._symbolRequestResponseHandler._markets, function(c, d) {
      $.each(d.submarkets, function(c, e) {
        $.each(e.symbols, function(c, e) {
          ...
          (a._symbolsInfo[e.symbol] = a._symbolsInfo[e.display_name] = j),
            a._symbolsList.push(e.symbol);
        });
      });
    }),
  ...
  (Datafeeds.SymbolsStorage.prototype.resolveSymbol = function(a, b, c) {
    "use strict";
    this._symbolsInfo.hasOwnProperty(a)
      ? b(this._symbolsInfo[a])
      : c("invalid symbol");
  }),
  ...
  (Datafeeds.SymbolSearchComponent.prototype.searchSymbolsByName = function(
    ...
    for (
      var c = this._datafeed._symbolsStorage,
        d = [],
        e = !a.ticker || 0 === a.ticker.length,
        f = 0;
      f < c._symbolsList.length;
      ++f
    ) {
      var g = c._symbolsList[f],
        h = c._symbolsInfo[g];
      if (
        !(a.type && a.type.length > 0 && h.type !== a.type) &&
        ((e || 0 === h.name.toUpperCase().indexOf(a.ticker)) &&
          d.push({
            symbol: h.name,
  ...
  (BinaryWebsockets = function() {
    "use strict";
    (this.unresolved_promises = []),
      (this.callbacks = []),
      (this.ws = null),
      (this._commonUtils = CommonUtils.getInstance()),
      (this.globalNotifier = GlobalNotifier.getInstance()),
      (this.reqIdCounter = 0);
  }),
  (BinaryWebsockets.prototype.init = function() {
    ...
    return (
      (this.ws = new WebSocket(
        "wss://frontend.binaryws.com/websockets/v3?l=en&app_id=2742"
      )),
      (this.ws.onopen = function(b) {
      ...
      (this.ws.onclose = function(b) {
        setTimeout(function() {
          a.init().then(function(b) {
            b &&
              (a.callbacks.ohlc || []).forEach(function(b) {
                if (b.requestObject) {
                  var c =
                      b.requestObject.req_id &&
                      Object.keys(b.requestObject).length > 1,
                    d =
                      !b.requestObject.req_id &&
                      Object.keys(b.requestObject).length > 0;
      ...
      (this.ws.onerror = function(b) {
        a.globalNotifier.noConnectionNotification(),
          $.growl.error({ message: "Connection error. Refresh page!" }),
          (a.unresolved_promises = []),
          (a.callbacks = []);
      }),
      (this.ws.onmessage = function(b) {
        ...
        e &&
          (c.error
            ? ((c.error.echo_req = c.echo_req), e.reject(c.error))
            : e.resolve(c),
          delete a.unresolved_promises[d]);
      }),
      new Promise(function(b, c) {
        (a.unresolved_promises.connectionOpenEvent =
          a.unresolved_promises.connectionOpenEvent || []),
          a.unresolved_promises.connectionOpenEvent.push({
            resolve: b,
            reject: c
          });
      })
  ...
  (BinaryWebsockets.prototype.send_request = function(a) {
    ...
    return c
      ? new Promise(function(c, d) {
          (b.unresolved_promises[a.req_id] = { resolve: c, reject: d }),
  ...
  (BinaryWebsockets.prototype.request_stop_ohlc_streaming = function(a, b) {
    ...
    var c = this;
    return (
      (this.callbacks.ohlc = this.callbacks.ohlc || []),
      this.callbacks.ohlc.forEach(function(a, d) {
  ...
  (BinaryWebsockets.prototype.request_ohlc_streaming = function(a, b, c) {
    ...
    this.ws.send(JSON.stringify(d)),
      c && ((c.requestObject = d), this.on("ohlc", c));
  }),
  (BinaryWebsockets.prototype.request_candles = function(a) {
    ...
    if (a.startTime) d = moment.utc(a.startTime);
      ...
      d = d.subtract(
        b * this._commonUtils.totalSecondsInABar(e.suffix, e.intVal),
        "seconds"
      );
    }
    var f = moment.utc();
    (f = f.subtract(3, "years")),
      (f = f.add(2, "days")),
      d.isBefore(f) && (d = f);
    var g = {
    ...
    return (
      (void 0 !== a.adjust_start_time && null !== a.adjust_start_time) ||
        (g.adjust_start_time = 1),
      this.send_request(g)
    );
  });
var CommonUtils = (function() {
    ...
    (a.prototype.parseSuffixAndIntValue = function() {
      "use strict";
      var a = TradingView.actualResolution
          .toUpperCase()
          .replace("D", "")
          .replace("M", "")
          .replace("W", ""),
...
(HistoricalOHLCReqRespHandler = function(a) {
  ...
  (HistoricalOHLCReqRespHandler.prototype.getBars = function(a, b, c) {
    ...
    this._binary_websockets
      .request_candles({ symbol: a.ticker, granularity: g })
      .catch(function() {
        c();
      })
      .then(function(a) {
  ...
  (OHLCStreamingReqResHandler = function(a, b) {
    (this._binary_websockets = a),
      (this._streamingMap = {}),
      (this._commonUtils = CommonUtils.getInstance()),
      (this._symbolRequestResponseHandler = b),
      (this.globalNotifier = GlobalNotifier.getInstance());
  }),
  (OHLCStreamingReqResHandler.prototype.subscribeBars = function(a, b, c) {
    ...
    this._streamingMap[c] = {
      ...
      timerCallback: function() {
        ...
        d._binary_websockets
          ...
          .then(function(c) {
            c &&
              c.candles &&
              c.candles.forEach(function(c) {
                var d = 1e3 * parseInt(c.epoch),
                  e = parseFloat(c.open),
                  f = parseFloat(c.high),
                  g = parseFloat(c.low),
                  h = parseFloat(c.close),
                  i = { time: d, open: e, high: f, low: g, close: h };
                (!a.lastBar || i.time > a.lastBar.time) && (a.lastBar = i),
                  b(i);
              });
          });
      },
      streamingCallback: function(a) {
        ...
        if (
          e &&
          a.ohlc.symbol === e.symbol &&
          a.ohlc.granularity === e.granularity
        ) {
          ...
          f &&
            ...
            i &&
            j &&
            ((this.lastBar = { time: f, open: g, high: h, low: i, close: j }),
            b(this.lastBar));
        }
      }
    };
    var g = this._symbolRequestResponseHandler.findInstrumentObjectBySymbol(
      a.ticker
    );
    if (g)
      if (g.delay_amount > 0) {
        ...
        this._binary_websockets.request_ohlc_streaming(a.ticker, f, {
          listenerID: c,
  ...
  (OHLCStreamingReqResHandler.prototype.unsubscribeBars = function(a) {
    ...
    b.timerHandler
      ? clearInterval(b.timerHandler)
      : this._binary_websockets
          .request_stop_ohlc_streaming(b.server_request_id, a)
          .then(function() {})
          .catch(function() {}),
  ...
  (SymbolReqRespHandler.prototype.init = function() {
    ...
    return new Promise(function(b, c) {
      a._binary_websockets
        .request_trading_times()
        .then(function(c) {
          a.process(c), b(a._symbolTypes, a._markets);
        })
        .catch(function(a) {
          "undefined" != typeof trackJs &&
            trackJs.track(
              "Unexpected response from server, [request_trading_times] Response error " +
                JSON.stringify(a)
            );
        });
    });
  ...
  (SymbolReqRespHandler.prototype.process = function(a) {
    ...
    for (var b = 0; b < a.trading_times.markets.length; b++) {
      ...
      for (
        var d = { name: c.name, submarkets: [] }, e = 0;
        e < c.submarkets.length;
        ++e
      ) {
        for (
          var f = c.submarkets[e], g = { name: f.name, symbols: [] }, h = 0;
          h < f.symbols.length;
          h++
        ) {
          var i = f.symbols[h];
          (i.feed_license && "chartonly" === i.feed_license) ||
            g.symbols.push({
              symbol: i.symbol,
  ...
  (SymbolReqRespHandler.prototype.findInstrumentObjectBySymbol = function(a) {
    var b = null,
      c = !0;
    return (
      this._markets.forEach(function(d) {
        return (
          d.submarkets.forEach(function(d) {
            return (
              d.symbols.forEach(function(d) {
                return (
                  d.symbol === a && ((b = $.extend(!0, {}, d)), (c = !1)), c
                );
              }),
              c
            );
